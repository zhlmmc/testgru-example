...
describe('UserManager', () => {
  ...
  describe('editUser', () => {
    ...
    it('should handle empty strings in edit', () => {
      const user = userManager.addUser('John', 'john@example.com');
      const editedUser = userManager.editUser(user.id, '', '');

      expect(editedUser?.name).toBe('');
      expect(editedUser?.email).toBe('');
    });
  });
  ...
  describe('findUsersByName', () => {
    ...
    it('should handle empty name search', () => {
      ...
      const foundUsers = userManager.findUsersByName('');
